...
void zip_file::save(const std::function<void(const char* data, size_t size)> &consumer)
{
    ...
    append_comment();
    consumer(buffer_.data(), buffer_.size());
}
...
{
    save([&stream](const char *data, size_t size){
...
{
    save([&bytes](const char *data, size_t size){
